...
"get_items" : function(doc, cdt ,cdn){
	...
	frappe.call({
		method: "agi.agi_project.doctype.combine_deliveries.combine_deliveries.get_delivery_note",
		args : {"data":lst},
		callback: function(r){
			if(r && r.message){
				...
				for(var i=0;i<data.length;i++){
					var child_doc = frappe.model.get_new_doc("Combine Deliveries Item", parent_doc, "result_table");
					var item = data[i]
					$.extend(child_doc, {
				...
				for(var i=0; i<item_wise_data.length; i++){
					var child_doc = frappe.model.get_new_doc("Combine Deliveries Itemwise", parent_doc, "item_wise_quantities");
...
"make_sales_return" : function(){
				...
				if(save_doc.length > 0){
					...
					frappe.call({
						method: "agi.agi_project.doctype.combine_deliveries.combine_deliveries.make_return_deliveries",
